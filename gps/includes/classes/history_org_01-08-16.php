...
class history {
	...
	static public function condition() {
		...
		$start_utc = gmdate('Y-m-d H:i:s', strtotime("{$_GET['date']}"));
		$end_utc = gmdate('Y-m-d H:i:s', strtotime("{$_GET['date']} +1 day"));
	...
	static public function calendar_json($unit_id, $year, $month) {
		...
		$monthstart_utc = gmdate('Y-m-d H:i:s', strtotime("{$year}-{$month}-01"));
		$monthend_utc = gmdate('Y-m-d H:i:s', strtotime("{$year}-{$month}-01 +1 month"));
