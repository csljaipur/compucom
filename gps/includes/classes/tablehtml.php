...
$btn_addnew = btn::create('Add ....', btn::TYPE_LINK, $link_addnew, '', '', 'Add ....');
...
function callback_tabledatahtml($tablehtml, $limit_offset, $limit_count, $query_order) {
	...
	while ($data = $db->record_fetch($result)) {
		...
		$btn_delete = btn::create('Delete', btn::TYPE_LINK, '#', '', $onclick_js, "Delete example \"{$name_js}\"");

		//Edit button
		$link_edit = navpd::forward(array('p' => 'example_addedit', 'edit' => $data['id']));
		$btn_edit = btn::create('Edit', btn::TYPE_LINK, $link_edit, '', '', 'Edit');
			...
			if (in_array($colalias, $this->sortable_columns)) {
				...
				if ($this->sortby == $colalias) {
					...
					if ($this->sortdir == 'asc') {
						...
						//$sortbyh = '/\\';
						$sortbyh = <<<EOHTML
 <img src="resources/maintable/sort_{$sortcol}_desc.gif" alt="Sorted descending">
EOHTML;
						...
						//$sortbyh = '\\/';
						$sortbyh = <<<EOHTML
 <img src="resources/maintable/sort_{$sortcol}_asc.gif" alt="Sorted ascending">
EOHTML;
...
EOHTML;
	...
	public function html_table_nav($right_html='') {
