...
//INSERT INTO `pt_unit`(`id`, `user_id`, `name`, `imei`, `password`, `icon`, `linecol`) VALUES ([value-1],[value-2],[value-3],[value-4],[value-5],[value-6],[value-7])		
	$registerit = "INSERT INTO `pt_unit`(`user_id`, `name`, `imei`, `password`, `icon`, `linecol`) VALUES (1,'Unknown','".$userentry."','".$userentry."123',0,0)";
	...
	if($row_check = mysql_fetch_array($result_check))
	{
		...
		$registermessage .= "Already Register, removed from Unregister";
	}
	else
	{
		$result = mysql_query($registerit);
		$result_del = mysql_query($deletit);
		$registermessage .= "Successfully Register, removed from Unregister";
	}
