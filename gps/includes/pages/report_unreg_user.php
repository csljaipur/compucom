...
*/		//alert(message);
	...
	function popupRemoveWindow(event,id,mobileno) // obj,messagebox,messagediv,mobileno,name)
	{
		//alert(id+" "+mobileno);
		
		document.getElementById("removemessagediv").innerHTML = "";
		document.getElementById('remove_mobilenopos').innerHTML = "";		

		document.getElementById("remove_unreg_userid").value = id;
		document.getElementById("remove_hiddenMobileNo").value = mobileno;		
		document.getElementById('remove_mobilenopos').innerHTML = mobileno;
		
		var x = event.clientX;
		var y = event.clientY;

		//alert("HELLO x = "+x+"  y = "+y);
		var d = document.getElementById('removePopup');
		d.style.position = "absolute";
		d.style.left = (x-330)+'px';
		d.style.top= (y-105)+'px';
		d.style.display='block'; 
	}	
	...
	function removeit()
	{
		var id = document.getElementById('remove_unreg_userid').value;	
		var mobileno = document.getElementById('remove_hiddenMobileNo').value;		
		$("#removemessagediv").load("includes/pages/removeit.php", {id:id, userentry:mobileno}, function(){
			$("#loadUsers").load("includes/pages/getUnregUsers.php");
			$("#removePopup").fadeOut(5000);			
		});
	}
...
<div id="removePopup" style="display:none;">	
	<div class="removemessage" id="removemessagediv"></div>
	<p>Mobile No&nbsp;&nbsp; : <span id="remove_mobilenopos"></span></p>  
	<input type="hidden"  name="remove_unreg_userid" id="remove_unreg_userid"  />
	<input type="hidden"  name="remove_hiddenMobileNo" id="remove_hiddenMobileNo"  />
	<center><input class="btn" type="button"  id="rembtn" name="rembtn" value="Remove Entry" onclick="removeit()" />
			<input class="btn" type="button"  id="remclose" name="remclose" value="Close" onclick="CloseRemovePopup()" />
	</center>
</div>
</body>
</html>
