...
function calendar(obj_ele_id, div_ele_id) {
	...
	this.setdate = function(date_yyyy, date_mm, date_dd) {
		...
		//Retrieve entry's available for specified year / month
		this.retrieve_entryavail(this.calshow_year, this.calshow_month);
			...
			for (i2=0; i2 < 7; i2++) {
				...
				if ( (month_startsonday_index_id == day_curr_index_id) || (month_started == true) ) {
					...
					if (dayno <= month_total_days) {
						...
						//If entries available for day
						var name = this.calshow_year + "-" + this.calshow_month;
						if (this.calshow_day == dayno) {
							dayaddinclass += " dayselected";
						} else if ( (trackhist_avail[name] != undefined) && (inarray(trackhist_avail[name], dayno)) ) {
							dayaddinclass += " historyforday";

							day_html = "<a href=\"#\" onclick=\"" + obj_ele_id + ".setdate(" + this.calshow_year + ", " + this.calshow_month + ", " + dayno + "); " + obj_ele_id + ".changepage(); return false;\">" + dayno + "</a>";
						}
	...
	this.changepage = function() {
		document.location.href = currpagedate_starturl + "&date=" + this.calshow_year + "-" + this.calshow_month + "-" + this.calshow_day;
		//this.generate();
	}

	//Retrieve days history is available for specified year / month
	this.retrieve_entryavail = function(year, month) {

		var name = year + "-" + month;

		if (trackhist_avail[name] == undefined) {

			try {

				nocache_urladdon = "&nocache=" + new Date().getTime();
				geturl = baseurl + "&t=histcal&unit=" + currtrackerunit + "&year=" + year + "&month=" + month + nocache_urladdon;
				var loader = new net.ContentLoader(geturl, function() {

					eval("var jsondata = " + this.req.responseText);

					trackhist_avail[name] = jsondata["data"];

					//Regenerate calendar with coloured entries
					eval(obj_ele_id + ".generate();");

				});

			} catch (e) {
				alert(e.message + "\nWas trying to retrieve days for specified month " + month + " (year " + year + "), try clicking refresh");
			}
